...
from core.db import Team, Setting, crud
from core.db.schemas import TeamListModel, TeamDetailModel
from core.dependencies import get_team_by_id, get_current_user
from core.dependencies.path import PaginationDep
from core.routes.teams.shema import TeamData
...
async def get_team(team: Team = Depends(get_team_by_id)):
    ...
    schema.permissions = await team.granted_permissions(scopes_list=True)
    return schema
...
async def create_team(team_data: TeamData):
    ...
    for setting in team_data.settings:
        await set_setting_value(
            await Setting.get(id=setting.setting_id),
            setting.new_value, team=new_team
        )
