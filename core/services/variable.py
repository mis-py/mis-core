...
class VariableService(BaseService):
    ...
    async def validate_variable(self, variable: UpdateVariable, variable_obj: Variable):
        if not variable_obj:
            raise NotFound(f"Variable with ID '{variable.variable_id}' is not exist")

        try:
            return type_convert(value=variable.new_value, to_type=variable_obj.type)
        except ValueError:
            raise ValidationFailed(
                f"Can't convert value '{variable.new_value}' to '{variable_obj.type}' for Variable with ID '{variable.variable_id}'",
            )
