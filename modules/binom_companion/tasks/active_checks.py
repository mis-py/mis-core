...
# )
# async def check_lead_rate_for_geos(
#         ctx: AppContext,
#         domain_change_cooldown: int = 1800,
#         last_period_duration: int = 300,
#         previous_period_duration: int = 300,
#         diff_percent: int = 30,
...
#             continue
#
#         if not await is_change_domain_cooldown_pass(geo.name, domain_change_cooldown):
#             logger.debug(
#                 f'[{geo.name}] check skipped. Cooldown is: {domain_change_cooldown}sec.'
#             )
#             continue
#
#         lead_rate_decreased = await LeadRecordService().check_lead_rate(
#             geo=geo.name,
#             last_period=last_period_duration,
#             previous_period=previous_period_duration,
...
#             await increase_task_value(geo, 'check_lead_rate_for_geos', lead_decrease_check_coefficient)
#         else:
#             # decrease if lead rate same or more
#             await decrease_task_value(geo, 'check_lead_rate_for_geos', lead_decrease_check_coefficient)
#
#
...
# )
# async def check_time_since_last_lead(
#         ctx: AppContext,
#         time_since_last_lead: int = 60 * 5,
#         lead_decrease_check_coefficient: float = 1.0
# ):
#     """
...
#             continue
#
#         last_lead_time_by_geo = await LeadRecordService().check_last_lead_time_by_geo(geo, int(time_since_last_lead))
#         last_lead_time_by_geo_passed = last_lead_time_by_geo > time_since_last_lead
#
#     message = {
#         'geo_name': geo.name,
...
#         last_lead_time_by_user_tag = await LeadRecordService().check_last_lead_time_by_user_tag(user.value, int(time_since_last_lead))
#         last_lead_time_by_user_tag_passed = last_lead_time_by_user_tag > time_since_last_lead
#
...
#             )
#
#            await increase_task_value(geo, 'check_time_since_last_lead', lead_decrease_check_coefficient, context.module_proxy)
#         else:
#            await decrease_task_value(geo, 'check_time_since_last_lead', lead_decrease_check_coefficient, context.module_proxy)
#
...
#     start_date=datetime.now() + timedelta(seconds=10)
# )
# async def check_current_domains(
#         ctx: AppContext,
...
#         decrease_coefficient = float(proxy_fail_check_coefficient) if proxy_count > 1 else 1.0
#
